...
app.whenReady().then(() => {
  ...
  if(mainWindow){
  localShortcut.register(mainWindow, ['Ctrl+W', 'Cmd+W', 'Alt+F4', 'Cmd+Q', 'Ctrl+Shift+I', 'F12', 'Ctrl+Tab', 'Alt+Tab', 'Meta+Tab', 'Ctrl+Alt+Tab'], () => {
    // Do nothing – block it
  });
}
if (!app.requestSingleInstanceLock()) {
  app.quit(); // if another instance is already running, exit
} else {
  app.on('second-instance', (event, argv, workingDirectory) => {
    // Focus mainWindow if a second instance tries to open
    if (mainWindow) {
      if (mainWindow.isMinimized()) mainWindow.restore();
      mainWindow.focus();

      const deepLinkArg = argv.find(arg => arg.startsWith('electron-app://'));
